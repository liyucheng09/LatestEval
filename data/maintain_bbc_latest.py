...
if __name__ == '__main__':
    ...
    first_day_of_this_month_str = first_day_of_this_month.strftime('%Y-%m-%d')

    year_str = today.strftime('%Y')
    month_str = today.strftime('%m')

    config = DatasetConfiguration()
    config.start_date = first_day_of_this_month
    ...
    # flatten the list of dicts to a dict of lists
    all_articles = {key: [article[key] for article in all_articles] for key in all_articles[0].keys()}

    # save all articles to a json file
    with open(f'{year_str}-{month_str}.json', 'w') as f:
        json.dump(all_articles, f, ensure_ascii=False)
    
    from huggingface_hub import hf_hub_download, RepoCard, upload_file

    upload_file(
        path_or_fileobj = f'{year_str}-{month_str}.json',
        path_in_repo = f'articles/{year_str}-{month_str}.json',
        repo_id = 'RealTimeData/bbc_alltime',
        repo_type = 'dataset',
        token=hf_token,
    )

    file = hf_hub_download(repo_id="RealTimeData/bbc_alltime", filename="configs.txt", repo_type='dataset')
    with open(file) as f:
        times = f.read().splitlines()
    
    times.append(f'{year_str}-{month_str}')
